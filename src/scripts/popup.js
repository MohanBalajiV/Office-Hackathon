...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const AgentXApiKeyInput = document.getElementById('AgentXApiKey');
    ...
    const modelsByProvider = {
      ...
      AgentX: [
        { value: 'ft:gpt-4o-2024-08-06:qeagle-2::CWzzTqgs', label: 'ft:gpt-4o-2024-08-06:qeagle-2::CWzzTqgs' }
      ]
    ...
    if (AgentXApiKeyInput) {
      AgentXApiKeyInput.addEventListener('change', (e) => {
        storage.set({ AgentXApiKey: e.target.value });
      });
    }
    ...
    function updateApiKeyVisibility(selectedModel) {
      ...
      const AgentXContainer = document.getElementById('AgentXKeyContainer');

      if (providerSelect.value === 'groq') {
        groqContainer.style.display = 'block';
        openaiContainer.style.display = 'none';
        AgentXContainer.style.display = 'none';
        ...
        groqContainer.style.display = 'none';
        openaiContainer.style.display = 'none';
        AgentXContainer.style.display = 'block';
      }
    }
    
    // Load saved values
    const result = await new Promise(resolve => {
      storage.get(['groqApiKey', 'openaiApiKey','AgentXApiKey', 'selectedModel', 'selectedProvider', 'githubToken', 'githubRepo', 'githubBranch'], resolve);
    });
    ...
    if (result.AgentXApiKey && AgentXApiKeyInput) {
      AgentXApiKeyInput.value = result.AgentXApiKey;
    }
